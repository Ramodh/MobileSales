...
namespace SageMobileSales.ServiceAgents.Services
{
    public class QuoteService : IQuoteService
    {
        ...
        public QuoteService(IServiceAgent serviceAgent, ILocalSyncDigestService localSyncDigestService, ILocalSyncDigestRepository localSyncDigestRepository,
            IQuoteRepository quoteRepository, ISalesRepRepository salesRepRepository, IQuoteLineItemRepository quoteLineItemRepository, IAddressRepository addressRepository, IEventAggregator eventAggregator)
        {
        ...
        {
            Constants.IsSyncAvailable = await _localSyncDigestService.SyncLocalDigest(Constants.QuoteEntity, Constants.syncDigestQueryEntity);
        ...
        private async Task SyncQuotes()
        {
            ...
            if (!string.IsNullOrEmpty(salesRepId))
            {
                ...
                quotesResponse = await _serviceAgent.BuildAndSendRequest(Constants.QuoteEntity, Constants.syncQueryEntity, null, Constants.AccessToken, parameters);
                if (quotesResponse != null && quotesResponse.IsSuccessStatusCode)
                {
                    ...
                    if (_returnedCount > 0 && _totalCount - _returnedCount >= 0 && !(DataAccessUtils.IsQuotesSyncCompleted))
                    {
        ...
        public async Task<Quote> SubmitQuote(Quote quote)
        {
            ...
            if (quote.QuoteId.Contains(Constants.Pending))
            {
                if (quote.AddressId.Contains(Constants.Pending))
                {
                    ...
                    await UpdateQuoteShippingAddress(quote, await _addressRepository.GetQuoteShippingAddress(quote.AddressId));
                }
                else
                {
                    quote = await PostQuote(quote);
            ...
            else
            {
                ...
                var result = quoteLineItemList.Where(e => e.QuoteLineItemId.Contains(Constants.Pending));
                ...
                quoteResponse = await _serviceAgent.BuildAndPutObjectRequest(quoteEntityId, null, Constants.AccessToken, parameters, obj);
                if (quoteResponse != null && quoteResponse.IsSuccessStatusCode)
                {
                    var sDataQuote = await _serviceAgent.ConvertTosDataObject(quoteResponse);
        ...
        public async Task DeleteQuote(string quoteId)
        {
            ...
            quoteResponse = await _serviceAgent.BuildAndDeleteRequest(quoteEntityId, null, Constants.AccessToken, parameters);
            if (quoteResponse != null && quoteResponse.IsSuccessStatusCode)
            {
        ...
        public async Task CalculateSalesTaxForQuote(Quote quote)
        {
            try
            {
                ...
                quoteResponse = await _serviceAgent.BuildAndSendRequest(Constants.QuoteEntity, queryEntity, null, Constants.AccessToken, parameters);
                if (quoteResponse != null && quoteResponse.IsSuccessStatusCode)
                {
                    var sDataQuote = await _serviceAgent.ConvertTosDataObject(quoteResponse);
            ...
            try
            {
                ...
                foreach (var quote in quotePendingList)
                {
                    if (quote.AddressId.Contains(Constants.Pending))
                    {
                        ...
                        if (_quote != null)
                            await UpdateQuoteShippingAddress(quote, await _addressRepository.GetQuoteShippingAddress(_quote.AddressId));
                    }
                    else
                    {
            ...
            try
            {
                ...
                foreach (var shippingAddress in shippingAddressDetails)
                {
                    if (shippingAddress.QuoteId.Contains(Constants.Pending))
                    {
                        ...
                        quote = await PostQuote(quote);
                        if(quote != null)
                            await UpdateQuoteShippingAddress(quote, await _addressRepository.GetQuoteShippingAddress(shippingAddress.AddressId));
                    }
                    else
                    {
                        if (shippingAddress.AddressId.Contains(Constants.Pending))
                            await UpdateQuoteShippingAddress(await _quoteRepository.GetQuoteAsync(shippingAddress.QuoteId), await _addressRepository.GetQuoteShippingAddress(shippingAddress.AddressId));
                        else
                            await UpdateQuoteShippingAddressKey(await _quoteRepository.GetQuoteAsync(shippingAddress.QuoteId));
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <returns></returns>
        private QuoteDetailsShippingAddressJson ConvertQuoteWithShippingAddressToJsonFormattedObject(Quote quote, List<QuoteLineItem> quoteLineItemList, Address address)
        {
            QuoteDetailsShippingAddressJson quoteJsonObject = new QuoteDetailsShippingAddressJson();
        ...
        /// <param name="quoteLineItemList"></param>
        /// <returns></returns>
        private QuoteDetailsShippingAddressKeyJson ConvertQuoteWithShippingAddressKeyToJsonFormattedObject(Quote quote, List<QuoteLineItem> quoteLineItemList)
        {
            QuoteDetailsShippingAddressKeyJson quoteJsonObject = new QuoteDetailsShippingAddressKeyJson();
            quoteJsonObject.Description = quote.QuoteDescription == null ? "" : quote.QuoteDescription;
            quoteJsonObject.DiscountPercent = quote.DiscountPercent;
            quoteJsonObject.QuoteTotal = quote.Amount;
            quoteJsonObject.SandH = quote.ShippingAndHandling;
