   ...
   public class ProductService : IProductService
    {
        ...
        public ProductService(ILocalSyncDigestService localSyncDigestService, IServiceAgent serviceAgent, ILocalSyncDigestRepository localSyncDigestRepository, IProductRepository productRepository)
        {
        ...
        {
          Constants.IsSyncAvailable=  await _localSyncDigestService.SyncLocalDigest(Constants.ItemsEntity ,Constants.syncDigestQueryEntity);
        ...
        private async Task SyncProducts()
        {
            try
            {
                LocalSyncDigest digest = await _localSyncDigestRepository.GetLocalSyncDigestDtlsAsync(Constants.ItemsEntity);
                ...
                productsResponse = await _serviceAgent.BuildAndSendRequest(Constants.ItemsEntity, Constants.syncQueryEntity, null, Constants.AccessToken, parameters);
                if (productsResponse!=null && productsResponse.IsSuccessStatusCode)
                {
                    ...
                    if (Convert.ToInt32(sDataProducts.GetNamedNumber("$totalResults")) > DataAccessUtils.ProductTotalCount)
                        DataAccessUtils.ProductTotalCount = Convert.ToInt32(sDataProducts.GetNamedNumber("$totalResults"));
                    ...
                    if (_returnedCount > 0 && _totalCount - _returnedCount >= 0 && !(DataAccessUtils.IsProductSyncCompleted))
                    {
                        ...
                        int _syncEndpointTick = Convert.ToInt32(lastProductObject.GetNamedNumber("SyncEndpointTick"));
