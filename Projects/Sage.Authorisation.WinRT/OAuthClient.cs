...
namespace Sage.Authorisation.WinRT
{
    ...
    ///  <para>
    ///    The <see cref="OAuthClient">OAuthClient</see> class handles all the complications of OAuth 2.0, including requesting and managing client credentials, refresh tokens,
    ///    access tokens and their expiry. It also presents the interactive user of the system with a secure dialog with which to authenticate
    ///    with Sage ID if required. To help mitigate against spyware and malware, the authentication dialog is started in its own Windows Desktop that is
    ...
    public sealed class OAuthClient
    {
        ...
        /// </summary>
        /// <param name="clientId">The client id.</param>
        /// <param name="supressInteractive">if set to <c>true</c> then the client will throw an exception if the authorisation needs user intervention (i.e. access token and refresh token are expired).</param>
        ...
        ///<summary>
        ///  <para>
        ///    The client ID you were provided with on registration. Please contact SSDPDeveloperSupport@sage.com to obtain a valid client identifier.
        ///  </para>
        ///  <para>
        ///    This property can only be set before an initial call is made to either
        ///    <see cref="Sage.Authorisation.WinRT.OAuthClient.AuthoriseAsync">Authorise</see> or <see cref="Sage.Authorisation.WinRT.OAuthClient.CleanupAsync">Cleanup</see>.
        ///  </para>
        ///</summary>
        ...
        ///    return cached access tokens, or attempt to retrieve new access tokens using stored refefresh tokens, if available. In the situation where no tokens can be retrieved
        ///    without prompting the interactive user for credentials or confirmation, and the SuppressInteractive property is set to true, <see cref="Sage.Authorisation.WinRT.Exceptions.InteractiveAuthorisationRequiredException">InteractiveAuthorisationRequiredException</see>
        ...
        public bool SuppressInteractive
        {
            get
            {
                return _suppressInteractive;
            }
            set
            {
                if (_busy)
                {
                    throw new Exceptions.AuthorisationBusyException();
                }
        ...
        /// </item>
        /// <item>
        /// <description>Stores a client credential (X.509 certificate and private key) in the windows certificate store</description>
        /// </item>
        /// <item>
        ...
        /// </item>
        /// <item>
        /// <description>Presents a secure desktop to the interactive user to authenticate and authorise operations.</description>
        /// </item>
        /// <item>
        ...
        ///   </code>
        ///   </example>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.AuthorisationBusyException">An authorisation attempt is already in progress.</exception>
        /// <exception cref="ArgumentNullException"><paramref name="startInfo" /> is null.</exception>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.CommunicationException">Sage.Authorisation library could not communicate with the authorisation server.</exception>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.ExpectedFoundException">The authorisation server did not respond with an expected HTTP 302 response.</exception>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.ExpectedLocationHeaderException">The authorisation server did not include a location with an HTTP 302 response.</exception>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.AuthorisationErrorResponseException">The authorisation server returned an error code during the authorisation process.</exception>
        /// <exception cref="Sage.Authorisation.WinRT.Exceptions.InteractiveAuthorisationRequiredException">No cached access token was available and no valid refresh token was stored for the current client ID and scope which would allow the silent retrieval of a new access token.</exception>
        /// <remarks>
        ///   <para>
        ...
        ///   <para>
        /// Information about the progress of an Authorise call is visible by subscribing to the <see cref="LogEvent">LogEvent</see> event.
        ///   </para>
        /// </remarks>
        ...
        ///</summary>
        ///<remarks>The runtime component comes baked with the default settings for the environment it targets but it may be required to override these settings, since Windows Store applications don't provide an app.config file we override the defaults by providing a configuration object here.</remarks>
        ///<param name="configuration">The configuration.</param>
        ///<exception cref="Sage.Authorisation.WinRT.Exceptions.AuthorisationBusyException">You can't override the configuration while an authorisation is in progress.</exception>
        public void SetConfiguration(Configuration configuration)
        {
            if (_busy)
            {
                throw new AuthorisationBusyException(_loader.GetString("ExceptionAuthorisationBusyException_SetConfiguration"));
            }
        ...
        private async Task<AuthorisationResult> AuthoriseUserAsync(AuthorisationInfo currentAuthorisation)
        {
            ...
            Uri resultUri = await _httpHelper.AuthenticateUsingBrokerAsync(brokerUri, new Uri(_configuration.RedirectUri));

            var formDecoder = new WwwFormUrlDecoder(resultUri.Query);

            // match expecetd uris
            if (StringComparer.OrdinalIgnoreCase.Equals(resultUri.GetComponents(UriComponents.SchemeAndServer | UriComponents.Path, UriFormat.SafeUnescaped)
                , _configuration.RedirectUri))
            {
                _log.Diagnostic(LogEventType.RedirectUriFound, String.Format(_loader.GetString("LogRedirectUriFound"), resultUri.ToString()));
                ...
                IWwwFormUrlDecoderEntry accessCodeParameter = formDecoder.FirstOrDefault(x => x.Name == _configuration.StartAuthorisationResponseType);
                ...
                IWwwFormUrlDecoderEntry requestCredentialParameter = formDecoder.FirstOrDefault(x => x.Name == Configuration.RedirectUriCred);
                if (requestCredentialParameter != null && bool.TryParse(requestCredentialParameter.Value, out requestCredential))
                {
                    await _certificateStore.RetrieveAndImportCredentialAsync(accessCodeParameter.Value, currentAuthorisation.DeviceName, currentAuthorisation.State);
                }

                AccessToken accessToken = await _tokenStore.GetAndStoreTokensAsync(accessCodeParameter.Value, currentAuthorisation.Scope, _configuration.RedirectUri, currentAuthorisation.State);
        ...
        private async Task<AuthorisationResult> DoAuthorisationAsync(AuthorisationInfo authorisationInfo)
        {
            ...
            try
            {
                ...
                AuthorisationResult currentResult = new AuthorisationResult()
                {
        ...
        /// </summary>
        /// <exception cref="InteractiveAuthorisationRequiredException">Client requested to SuppressInteractive authorisations but an interactive authorisation is required.</exception>
        /// <param name="currentAuthorisationInfo"></param>
        /// <returns></returns>
        private async Task<AuthorisationResult> PerformInteractiveAuthorisationAsync(AuthorisationInfo currentAuthorisationInfo)
        {
            if (_suppressInteractive)
            {
                throw new Exceptions.InteractiveAuthorisationRequiredException(currentAuthorisationInfo.State);
            }
        ...
        /// <returns></returns>
        private bool TryAuthoriseFromExistingAccessToken(AuthorisationInfo currentAuthorisation, out AuthorisationResult result)
        {
            result = null;
        ...
        /// <returns></returns>
        private async Task<AuthorisationResult> TryAuthoriseFromRefreshTokenAsync(AuthorisationInfo currentAuthorisationInfo)
        {
            try
            {
