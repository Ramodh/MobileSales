...
namespace Sage.Authorisation.WinRT.Exceptions
{
    ...
    /// </summary>
    /// <remarks>
    /// If this exception occurs, the Sage ID server has rejected the authorisation attempt. You should examine the Error and
    /// ErrorDescription properties to see the reason for the failure. See the following list for types of error and error description:
    /// <list type="table">
    ///    <listheader>
    ...
    ///        <description>The request is missing a required parameter, includes an invalid parameter value, or is otherwise malformed.</description>
    ///    </item>
    ///    <item>
    ///        <term>unauthorized_client</term>
    ///        <description>The client is not authorized to request an authorisation code using this method / The authenticated client is not authorized to use this authorisation grant type.</description>
    ///    </item>
    ///    <item>
    ...
    ///        <description>The authorisation server encountered an unexpected condition which prevented it from fulfilling the request.</description>
    ///    </item>
    ///    <item>
    ///        <term>temporarily_unavailable</term>
    ///        <description>The authorisation server is currently unable to handle the request due to a temporary overloading or maintenance of the server.</description>
    ///    </item>
    ///    <item>
    ///        <term>invalid_client</term>
    ///        <description>Client authentication failed (e.g. unknown client, no client authentication included, or unsupported authentication method). The authorisation server MAY return an HTTP 401 (Unauthorized) status code to indicate which HTTP authentication schemes are supported. If the client attempted to authenticate via the "Authorisation" request header field, the authorisation server MUST respond with an HTTP 401 (Unauthorized) status code, and include the "WWW-Authenticate" response header field matching the authentication scheme used by the client.</description>
    ///    </item>
    ///    <item>
    ///        <term>invalid_grant</term>
    ///        <description>The provided authorisation grant (e.g. authorisation code, resource owner credentials) or refresh token is invalid, expired, revoked, does not match the redirection URI used in the authorisation request, or was issued to another client.</description>
    ///    </item>
    ///    <item>
    ...
    ///    </item>
    /// </list>
    /// <para>The following is a list of sub-error codes, which are specific to the Sage ID implementation of OAuth 2.0. These are available in the ErrorDescription property of the exception:</para>
    /// <list type="table">
    ///    <listheader>
    ...
    ///        <description>The client supplied a redirect uri that is not identical to the redirect uri supplied in the initial authorisation request.</description>
    ///    </item>
    ///    <item>
    ///        <term>unexpected_redirect_uri</term>
    ///        <description>The client must not include a redirect uri unless they included the same redirect uri in the initial authorisation request.</description>
    ///    </item>
    ///    <item>
    ...
    ///        <description>The client is not allowed to request at least one of the permission sets expressed in the scope parameter.</description>
    ///    </item>
    ///    <item>
    ...
    ///        <description>The client requested a permission set handle for a resource app that the client Id is not linked to.</description>
    ///    </item>
    ///    <item>
    ...
    ///        <description>A generic error to indicate that the user could not be signed in. We CANNOT be more specific than risk because we'd be leaking information about the existence of accounts.</description>
    ///    </item>
    ///    <item>
    ...
    internal class AuthorisationErrorResponseException : AuthorisationException
    {
        ...
        internal AuthorisationErrorResponseException(string message, string state, string error, string error_description)
            : base(message, state)
        {
            HResult = ExceptionHResults.AuthorisationErrorResponseException;
            Error = error;
        ...
        internal AuthorisationErrorResponseException(string message, string error, string error_description, Exception inner)
        ...
        {
            return String.Format("{0}\r\n\r\nError={1}, Description={2}.\r\n\r\n{3}", Message, Error, ErrorDescription, base.ToString());
        }
