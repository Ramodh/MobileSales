...
namespace Sage.Authorisation.WinRT
{
    ...
    /// <para>
    /// The <see cref="OAuthClientThin">OAuthClientThin</see> class is a cut down version of the OAuthClient. It presents the interactive
    /// user of the system with a secure dialog with which to authenticate. To help mitigate against spyware and malware, the authentication dialog
    /// is started in its own Windows Desktop that is independant of the desktop in which the current user is running their applications.
    /// </para>
    /// </summary>
    /// <example>
    ///   <code>
    ...
    public sealed class OAuthClientThin
    {
        ...
        ///<remarks>
        ///    <para>
        ///        In the thin client flow the BeginAuthorisationAttempt step isn't performed by the client, some other component performs this step and passes the URI to this method.
        ///    </para>
        ///    <para>
        ///        The result of this authorise call (the access code) should be passed back to the other component to obtain it's tokens and credentials.
        ///    </para>
        ...
        /// <remarks>
        /// The runtime component comes baked with the default settings for the environment it targets but it may be required to override these settings, since Windows Store applications don't provide an app.config file we override the defaults by providing a configuration object here.
        /// </remarks>
        ...
        private async Task<AuthorisationResultThinClient> DoAuthoriseAsync(string startUriString)
        {
            ...
            Uri resultUri = await _httpHelper.AuthenticateUsingBrokerAsync(startUri, new Uri(_configuration.RedirectUri));
                ...
                , _configuration.RedirectUri))
            {
                ...
                AuthorisationResultThinClient result = AuthorisationResultThinClient.CreateResult(true, accessCodeParameter.Value, requestCredential);
