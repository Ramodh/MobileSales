...
namespace Sage.Authorisation.WinRT
{
    ...
    /// <para>
    /// It contains the encrypted AccessToken, its expiry, and also the user state which was specified when the Authorise method was called with
    /// an instance of <see cref="AuthorisationInfo">AuthorisationInfo</see>.
    /// </para>
    /// </summary>
    /// <example>
    /// The following code snippet shows how to make a call, via REST, using the encrypted access token in an <see cref="AuthorisationResult">AuthorisationResult.</see>
    ///   <code>
    ///  <summary>
    ...
    ///   </example>
    /// <remarks>
    /// The <see cref="Sage.Authorisation.WinRT.AuthorisationResult.Expiry">Expiry</see> property gives the expiration of the access token in UTC.
    /// <para>It is not possible for the client application to decrypt the AccessToken. The encryption key is known only to the resource web application.</para>
    /// </remarks>
