...
namespace Sage.Authorisation.WinRT.Storage
{
    ...
    internal class CredentialStore
    {
        ...
        /// <remarks>There is no way to programatically delete the credentials, this application setting 
        /// may return false but a credential might actually exist. Clearing this flag will force a new credential 
        /// to be retrieved but the old credential may still be used if it's "in date" due to the automatic selection of credentials.</remarks>
        /// </summary>
        /// <returns>
        ...
        internal bool HasValidCredential()
        {
            ...
            long expiryAsUtcFileTime = (long)ApplicationData.Current.LocalSettings.Values[_clientId + CertificateExpiryKey];
        ...
        /// <remarks>Credentials are stored here... 
        /// C:\Users\{username}\AppData\Local\Packages\{AppPackageName}\AC\Microsoft\SystemCertificates\My\Certificates</remarks>
        /// </summary>
        ...
        internal async Task RetrieveAndImportCredentialAsync(string accessCode, string deviceName, string state)
        {
            ...
            CertificateMetadata certificate = await _httpHelper.GetClientCredentialAsync(accessCode, password, deviceName, state);
        ...
        {
            ApplicationData.Current.LocalSettings.Values[_clientId + CertificateExpiryKey] = certificate.expiry.ToFileTimeUtc();
        }
    }
}
