...
namespace Sage.Authorisation.WinRT
{
    ...
    public sealed class AuthorisationInfo
    {
        ...
        ///  the <see cref="Sage.Authorisation.WinRT.OAuthClient">AuthoriseAsync</see> call to return tokens for the longest possible time
        ...
        {
            StringBuilder template = new StringBuilder(hasClientCredential ? config.StartAuthorisationAttemptWithCredentialUri : config.StartAuthorisationAttemptUri);

            template.Replace("{RESPONSETYPE}", Uri.EscapeDataString(ResponseType));
            template.Replace("{CLIENTID}", Uri.EscapeDataString(ClientId));
            template.Replace("{REDIRECTURI}", Uri.EscapeDataString(config.RedirectUri.ToString()));
