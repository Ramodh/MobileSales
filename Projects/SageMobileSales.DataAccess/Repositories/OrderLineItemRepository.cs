...
namespace SageMobileSales.DataAccess.Repositories
{
    public class OrderLineItemRepository : IOrderLineItemRepository
    {
        ...
        /// <param name="orderId"></param>
        /// <returns></returns>
        public async Task<OrderLineItem> AddOrUpdateOrderLineItemJsonToDbAsync(JsonObject sDataOrderLineItem, string orderId)
        {
            try
            {
                IJsonValue value;
                if (sDataOrderLineItem.TryGetValue("$key", out value))
                {
                    if (value.ValueType.ToString() != DataAccessUtils.Null)
                    {
                        ...
                        orderLineItemList = await _sageSalesDB.QueryAsync<OrderLineItem>("SELECT * FROM OrderLineItem where OrderLineItemId=?", sDataOrderLineItem.GetNamedString("$key"));

                        if (orderLineItemList.FirstOrDefault() != null)
                        {
                            return await UpdateOrderLineItemJsonToDbAsync(sDataOrderLineItem, orderLineItemList.FirstOrDefault());
                        }
                        else
                        {
        ...
        public async Task<List<OrderLineItem>> GetPreviouslyPurchasedProducts(string customerId)
        {
            ...
            try
            {
                ...
                previouslyPurchasedProductList = await _sageSalesDB.QueryAsync<OrderLineItem>("SELECT distinct OLT.ProductId, MAX(OLT.price) as Price FROM OrderLineItem as OLT WHERE OLT.OrderId in (SELECT OrderId FROM Orders WHERE CustomerId=?) Group by OLT.ProductId", customerId);
        ...
        public async Task<List<OrderLineItem>> GetOrderLineItemForOrder(string orderId)
        {
            ...
            try
            {
                ...
                orderLineItemList = await _sageSalesDB.QueryAsync<OrderLineItem>("SELECT * FROM OrderLineItem WHERE OrderId=?", orderId);
        ...
        /// <returns></returns>
        private async Task<OrderLineItem> UpdateOrderLineItemJsonToDbAsync(JsonObject sDataOrderLineItem, OrderLineItem orderLineItemDbObj)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task<OrderLineItem> ExtractOrderLineItemFromJsonAsync(JsonObject sDataOrderLineItem, OrderLineItem orderLineItem)
        {
            try
            {
                IJsonValue value;
        ...
        private async Task DeleteOrderLineItemsFromDbAsync(JsonArray sDataOrderLineItemArray, string orderId)
        {
            ...
            try
            {
                ...
                {
                    var sDataOrderLineItem = orderLineItem.GetObject();
                    OrderLineItem orderLineItemJsonObj = new OrderLineItem();
                    if (sDataOrderLineItem.TryGetValue("$key", out value))
                    {
                        if (value.ValueType.ToString() != DataAccessUtils.Null)
                        {
                ...
                orderLineItemIdDbList = await _sageSalesDB.QueryAsync<OrderLineItem>("SELECT * FROM OrderLineItem where OrderId=?", orderId);
                ...
                for (int i = 0; i < orderLineItemIdDbList.Count; i++)
                {
                    ...
                    for (int j = 0; j < orderLineItemIdJsonList.Count; j++)
                    {
                        ...
                        else if (orderLineItemIdDbList[i].OrderLineItemId == orderLineItemIdJsonList[j].OrderLineItemId)
                        {
                            idExists = true;
                            break;
                        }
                ...
                {
                    foreach (var orderLineItemRemove in orderLineItemRemoveList)
                    {
